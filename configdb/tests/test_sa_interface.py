...
class TestSaInterface(TestBase):
    ...
    def init_db(self):
        return sa_interface.SqlAlchemyDbInterface(self.dburi, self.schema)
    ...
    def test_find(self):
        ...
        r = db.find('host', {'roles': [{'type': 'eq', 'arg': 'role1'}], 'name': {'type': 'eq', 'arg': 'obz'}}).all()
